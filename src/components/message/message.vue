<template>
  <mu-list>
    <!-- 设置列表删除时动态效果 -->
    <div v-for="(item, index) of friends" :class="[{swipeleft: isSwipe[index]},'wrap']" @click="getActiveId_x(item._id,index)" ref="child" :key="index">
      <mu-list-item :title="item.name" :describeLine="1" :disableRipple="true" class="list-item">
        <!-- 头像 -->
      ...
      <!-- 分割线 -->
      <div class="delete" @click.stop="deleteM(index)">删除</div>
    </div>
  </mu-list>
</template>
...
<style lang="scss" scoped>
  ...
  .mu-list{
    ...
    .wrap{
      ...
      // 引入mixin函数
      @include border-b-1px(rgba(0,0,0,.1))
      .list-item{
        float: left;
        width: 85%;
        height: 10vh;
